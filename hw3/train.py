...
parser.add_argument("--lr", type=int, default=2.5e-4)
...
parser.add_argument("--ckpt_path", type=str, default="./ckpt/model.best")
...
class T5_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.split == "train":
            return tokenizer.batch_encode_plus([self.clean(self.p[idx])], max_length=args.max_length, truncation = True, padding="max_length", return_tensors="pt").input_ids, tokenizer.batch_encode_plus([self.clean(self.titles[idx])], max_length=args.max_length, truncation = True, padding="max_length", return_tensors="pt").input_ids
        elif self.split == "dev":
...
if args.phase == "train":
    ...
    for epoch in tqdm.tqdm(range(args.num_epoch)):
        ...
        model.train()
        train_loss = 0
        for p, title in tqdm.tqdm(train_dataloader):
            step += 1
            p, title = p.to(args.device).squeeze(1), title.to(args.device).squeeze(1)
            loss = model(input_ids = p, labels = title).loss
            loss /= accu_step
            loss.backward()
        ...
        for p, title in tqdm.tqdm(dev_dataloader):
            ...
            output = list(map(lambda x:x.strip(),tokenizer.batch_decode(model.generate(p, max_length=args.max_length, repetition_penalty = 2.5, num_beams = 5),skip_special_tokens=True, clean_up_tokenization_spaces=True)))
            print(output)
            if output[0]:
                res.extend(output)
                titles.extend(title)
        torch.save(model.state_dict(), f"./ckpt/{epoch}.ckpt")
        eval_res = get_rouge(res, titles)
        print(eval_res)
        with open(f"./statistic/valid_{epoch}.json", "w") as fp:
            json.dump(eval_res, fp, indent = 4)
        if eval_res["rouge-l"]["f"] >= best_f:
            print("Update best model")
            best_f = eval_res["rouge-l"]["f"]
            torch.save(model.state_dict(), f"./ckpt/model.best")
else:
    try:
        model.load_state_dict(torch.load(args.ckpt_path, map_location=args.device))
    except:
    ...
    with open(args.out_file, "w", encoding="utf8") as fp:
        for p, idx in tqdm.tqdm(test_dataloader):
            p = p.to(args.device).squeeze(1)
            output = list(map(lambda x:x.strip(),tokenizer.batch_decode(model.generate(p, max_length=args.max_length,num_beams=5, repetition_penalty=3.0),skip_special_tokens=True, clean_up_tokenization_spaces=True)))
            print(output)
