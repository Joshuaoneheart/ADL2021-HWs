...
parser.add_argument("--ckpt_path", type=str, default="./ckpt/hybrid")
...
if args.phase == "train":
    ...
    for epoch in tqdm.tqdm(range(args.num_epoch)):
        ...
        for p, title, text in tqdm.tqdm(train_dataloader):
            ...
            for idx, logit in enumerate(logits[0]):
                logit = Categorical(logits=logit)
                action.append(logit.sample())
                if policys == None:
                    policys = logit.log_prob(action[-1]).unsqueeze(0)
                else:
                    policys = torch.cat([policys, logit.log_prob(action[-1]).unsqueeze(0)])
            ...
            loss += 0.1 * torch.sum(torch.mul(policys, Variable(rewards)).mul(-1),-1)
            loss /= accu_step
            loss.backward()
            ...
            if step % args.log_step == 0:
                ...
                with open(f"./statistic/hybrid/loss_{epoch}", "a") as fp:
        ...
        for p, title in tqdm.tqdm(dev_dataloader):
            ...
            output = list(map(lambda x:x.strip(),tokenizer.batch_decode(model.generate(p, max_length=args.max_length, repetition_penalty = 3.0, num_beams = 5, temperature = 0.8),skip_special_tokens=True, clean_up_tokenization_spaces=True)))
            print(output)
            if output[0]:
                res.extend(output)
                titles.extend(title)
        ...
        print(eval_res)
        with open(f"./statistic/hybrid/valid_{epoch}.json", "w") as fp:
    ...
    with open(args.out_file, "w", encoding="utf8") as fp:
        for p, idx in tqdm.tqdm(test_dataloader):
            p = p.to(args.device).squeeze(1)
            output = list(map(lambda x:x.strip(),tokenizer.batch_decode(model.generate(p, max_length=args.max_length, num_beams = 5, repetition_penalty = 3.0, temperature= 0.8),skip_special_tokens=True, clean_up_tokenization_spaces=True)))
            print(output)
